...
object StringsStats {
  val xml1222 = """<?xml version="1.0" encoding="UTF-8"?>
<sense>
<stagk>案じる</stagk>
<pos>&v1;</pos>
<pos>&vt;</pos>
...
<gloss xml:lang="ger">(1) etw. festhalten</gloss>2
...
object Util {
  def EventReaderToList(xmlEventReader: XMLEventReader) : List[XMLEvent] = {
    def loop(xs : List[XMLEvent]) : List[XMLEvent] = {
      if (xmlEventReader.hasNext) loop(xmlEventReader.next() :: xs) else xs
    }
    loop(List.empty).reverse
  }
}
class SenseParserSpec extends FreeSpec with GivenWhenThen {
  "Sense" - {
    "Has glosses" in {
      val source = scala.io.Source.fromString(JDictXml.xml1)

      val headWordIndexer = new HeadWordIndexer(new XMLEventReader(source), List(entry => {
        assert(entry.sense(0).glosses("eng").toSet == Set(
